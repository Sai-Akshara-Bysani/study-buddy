...
joinBtn.addEventListener('click', async () => {
  ...
  if (!initSupabase()) {
    // Demo mode: show UI without real sync
  ...
  // Create a Supabase Realtime Presence channel for the room
  channel = supabase.channel(`study-room-${code}`, {
  ...
  channel
    .on('presence', { event: 'sync' }, () => {
      ...
      for (const [key, presences] of Object.entries(state)) {
        // Take the latest presence entry for each member
        const latest = presences[presences.length - 1];
        members[key] = { name: latest.name, status: latest.status || 'idle' };
      }
      renderFriends();
    })
...
window.broadcastStatus = async function(status) {
  ...
  if (channel) {
    ...
    if (members[myName]) members[myName].status = status;
    renderFriends();
  }
};
